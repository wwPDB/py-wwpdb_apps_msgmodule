...
class CircuitBreaker:
    ...
    def __init__(self, name: str, config: Optional[CircuitBreakerConfig] = None):
        ...
        self.logger.info(
            f"Circuit breaker '{name}' initialized with config: {self.config}"
        )
    ...
    def _execute_with_timeout(self, func: Callable, *args, **kwargs) -> Any:
        ...
        try:
            ...
            if execution_time >= self.config.timeout:
                raise CircuitBreakerTimeoutException(
                    f"Operation exceeded timeout of {self.config.timeout}s"
                )
    ...
    def _on_success(self):
        ...
        with self._lock:
            ...
            if self._state == CircuitBreakerState.HALF_OPEN:
                ...
                self.logger.debug(
                    f"Circuit breaker '{self.name}' half-open success count: {self._success_count}"
                )
    ...
    def _on_timeout(self):
        ...
        with self._lock:
            ...
            self._failure_count += 1
            self.logger.warning(
                f"Circuit breaker '{self.name}' recorded timeout {self._failure_count}"
            )
    ...
    def _close_circuit(self):
        ...
        if self._state != CircuitBreakerState.CLOSED:
            self.logger.info(
                f"Circuit breaker '{self.name}' closed, resuming normal operation"
            )
            self._state = CircuitBreakerState.CLOSED
            self._failure_count = 0
            self._success_count = 0
            ...
            return {
                ...
                "metrics": {
                    ...
                    * 100,
                    ...
                    if self.metrics.state_changed_time
                    else None,
                },
                "config": {
