...
#
from mmcif_utils.message.PdbxMessage import PdbxMessageInfo, PdbxMessageFileReference, PdbxMessageOrigCommReference, \
    PdbxMessageStatus
...
class MessagingIo(object):
    ...
    def __init__(self, reqObj, verbose=False, log=sys.stderr):
        ...
        #
        self.__expMethodList = (self.__reqObj.getValue("expmethod").replace('"', '')).split(",") if (
                len(self.__reqObj.getValue("expmethod").replace('"', '')) > 1) else []
        self.__emDeposition = True if (
                "ELECTRON MICROSCOPY" in self.__expMethodList or "ELECTRON CRYSTALLOGRAPHY" in self.__expMethodList) else False
        #
    ...
    def getMsgColList(self, p_bCommHstryRqstd=False):
        ...
        commHistoryAttributesLst = ['orig_message_id', 'orig_deposition_data_set_id', 'orig_timestamp', 'orig_sender',
                                    'orig_recipient', 'orig_message_subject', 'orig_attachments']
        ...
        try:
            ...
            if (rtrnList):
                ...
                if (self.__verbose):
                    self.__lfh.write(
                        "+%s.%s() -- Column list for message info returned as:\n %s\n" % (self.__class__.__name__,
                                                                                          sys._getframe().f_code.co_name,
                                                                                          rtrnList))
                #
        ...
        try:
            if self.__isWorkflow():
                ...
                if (contentType == "msgs" or bCommHstryRqstd):
                    self.__msgsFrmDpstrFilePath = msgDI.getFilePath(contentType='messages-from-depositor',
                                                                    format="pdbx")
            ...
            if (contentType == "msgs" or bCommHstryRqstd):
                ...
                if self.__msgsToDpstrFilePath is not None and os.access(self.__msgsToDpstrFilePath, os.R_OK):
                    ...
                    if (ok):
                        recordSetLst.extend(
                            pdbxMsgIo_toDpstr.getMessageInfo())  # in recordSetLst we now have a list of dictionaries with item names as keys and respective data for values
                ...
                if self.__notesFilePath is not None and os.access(self.__notesFilePath, os.R_OK):
                    ...
                    if (ok):
                        if (contentType == "notes"):
                            ...
                            onlyArchvdCommsLst = [record for record in fullNotesLst if
                                                  ("archive" in record['message_type'])]
    ...
    def getMsgRowList(self, p_depDataSetId, p_sSendStatus='Y', p_bServerSide=False, p_iDisplayStart=None,
                      p_iDisplayLength=None, p_sSrchFltr=None, p_colSearchDict=None, p_bThreadedRslts=False):
        ...
        try:
            ...
            if (contentType == "msgs" or bCommHstryRqstd):
                ...
                if self.__msgsToDpstrFilePath is not None and os.access(self.__msgsToDpstrFilePath, os.R_OK):
                    ...
                    if (ok):
                        ...
                        recordSetLst.extend(
                            msgsToDpstrLst)  # in recordSetLst we now have a list of dictionaries with item names as keys and respective data for values
            ...
            if p_bServerSide:
                ...
                iSortingCols = int(self.__reqObj.getValue('iSortingCols')) if self.__reqObj.getValue(
                    'iSortingCols') else 0
                #
                ordL = []
                descL = []
                for i in range(iSortingCols):
                    iS = str(i)
                    idxCol = int(self.__reqObj.getValue('iSortCol_' + iS)) if self.__reqObj.getValue(
                        'iSortCol_' + iS) else 0
                    sortFlag = self.__reqObj.getValue('bSortable_' + iS) if self.__reqObj.getValue(
                        'bSortable_' + iS) else 'false'
                    sortOrder = self.__reqObj.getValue('sSortDir_' + iS) if self.__reqObj.getValue(
                        'sSortDir_' + iS) else 'asc'
                    if sortFlag == 'true':
                        ...
                        colName = self.__reqObj.getValue('mDataProp_' + str(idxCol)) if self.__reqObj.getValue(
                            'mDataProp_' + str(idxCol)) else ''
        ...
        try:
            ...
            if self.__msgsToDpstrFilePath is not None and os.access(self.__msgsToDpstrFilePath, os.R_OK):
                ...
                with LockFile(self.__msgsToDpstrFilePath, timeoutSeconds=self.__timeoutSeconds,
                              retrySeconds=self.__retrySeconds, verbose=self.__verbose,
                              log=self.__lfh) as lf, FileSizeLogger(self.__msgsToDpstrFilePath, verbose=self.__verbose,
                                                                    log=self.__lfh) as fsl:
                    pid = os.getpid()
                    ok = mIIo2.read(self.__msgsToDpstrFilePath, "msgingmod" + str(pid))
                if (ok):
                    recordSetLst.extend(
                        mIIo2.getFileReferenceInfo())  # in recordSetLst we now have a list of dictionaries with item names as keys and respective data for values
            #
            if (self.__verbose and self.__debug and self.__debugLvl2):
                for idx, row in enumerate(recordSetLst):
                    self.__lfh.write("\n+%s.%s() -- row[%s]: %r\n" % (
    ...
    def getNotesList(self):
        ...
        try:
            ...
            if os.access(self.__notesFilePath, os.R_OK):
                mIIo = PdbxMessageIo(verbose=self.__verbose, log=self.__lfh)
                with LockFile(self.__notesFilePath, timeoutSeconds=self.__timeoutSeconds,
                              retrySeconds=self.__retrySeconds, verbose=self.__verbose,
                              log=self.__lfh) as lf, FileSizeLogger(self.__notesFilePath, verbose=self.__verbose,
                                                                    log=self.__lfh) as fsl:
                    pid = os.getpid()
    ...
    def processMsg(self, p_msgObj):
        ...
        try:
            ...
            if (
                    p_msgObj.isLive):  # if this a livemsg as opposed to draft then need to instantiate new PdbxMessageInfo object/acquire the new message data
            ...
            #
            mIIo = PdbxMessageIo(verbose=self.__verbose, log=self.__lfh)
            with LockFile(outputFilePth, timeoutSeconds=self.__timeoutSeconds, retrySeconds=self.__retrySeconds,
                          verbose=self.__verbose, log=self.__lfh) as lf, FileSizeLogger(outputFilePth,
                                                                                        verbose=self.__verbose,
                                                                                        log=self.__lfh) as fsl:
                pid = os.getpid()
                bGotContent = mIIo.read(outputFilePth, "msgingmod" + str(
                    pid))  # may return False if there was a file but the file had no content yet (i.e. no annotator messages yet)
            ...
            if (self._sanityCheck(p_msgObj.contentType, bGotContent, mIIo, outputFilePth)):
                ...
                if (p_msgObj.isLive):
                    ...
                    mIIo.update('pdbx_deposition_message_info', 'message_subject', p_msgObj.messageSubject,
                                iRow=rowToUpdate)
                ...
                if (
                        p_msgObj.isBeingSent and not self.__groupId):  # i.e. not executed for messages remaining in "draft" state
                    ...
                    if bSuccess:
                        ...
                        for mfr in msgFileRefs:
                            ...
                            if (
                                    mfr.getContentType() == 'validation-report-annotate' or mfr.getContentType() == 'validation-report-full-annotate'):
                ...
                if (
                        'archive' in p_msgObj.messageType or 'forward' in p_msgObj.messageType) and 'noorig' not in p_msgObj.messageType:
                    self._handleOrigCommReferences(p_msgObj, mIIo)

                self._updateSnapshotHistory(outputFilePth)

                with LockFile(outputFilePth, timeoutSeconds=self.__timeoutSeconds, retrySeconds=self.__retrySeconds,
                              verbose=self.__verbose, log=self.__lfh) as lf:
                ...
                if bOk and not self.__groupId:
                    if self.__isWorkflow() and p_msgObj.contentType == "msgs":
                        ...
                        with LockFile(depUiMsgsToDpstrFilePath, timeoutSeconds=self.__timeoutSeconds,
    ...
    def _getOutputFileTarget(self, contentType):
        ...
        msgDI = MessagingDataImport(self.__reqObj, verbose=self.__verbose, log=self.__lfh)
        if (contentType == "msgs"):
            returnFilePath = msgDI.getFilePath(contentType='messages-to-depositor',
                                               format="pdbx") if self.__isWorkflow() else self.__msgsToDpstrFilePath
            ...
            returnFilePath = msgDI.getFilePath(contentType='notes-from-annotator',
                                               format="pdbx") if self.__isWorkflow() else self.__notesFilePath
    ...
    def _sanityCheck(self, contentType, bGotContent, mIIo, outputFilePth):
        ...
        '''
        highWatermark = self.__reqObj.getValue(
            "msgs_high_watermark") if contentType == "msgs" else self.__reqObj.getValue("notes_high_watermark")
        bReturnVal = True

        if (highWatermark is not None and len(
                highWatermark) > 0):  # only if we have a valid high water mark can we peform the sanity check
            self.__lfh.write(
            ...
            try:
                ...
                assert (int(currentNumRows) >= int(highWatermark)), (
                        "+%s.%s() --  CRITICAL ERROR -- file at: %s, had %s records but should have had at least %s records!\n" % (
                    self.__class__.__name__,
                    sys._getframe().f_code.co_name,
    ...
    def autoMsg(self, p_depIdList, p_tmpltType="release-publ", p_isEmdbEntry=False, p_sender="auto"):
        ...
        for depId in p_depIdList:  # for depId,tmpltType in p_depIdList:
            ...
            self.__reqObj.setValue("identifier",
                                   depId)  # IMPORTANT: enforcing value of deposition ID for all subsequent downstream processing
            #
            # Added by ZF
            #
            self.__groupId = ''
            ...
            sAccessionIdString = templateDict['accession_ids_em_rel'] if p_isEmdbEntry else templateDict[
                'accession_ids']
        ...
        try:
            ...
            if self.__notesFilePath is not None and os.access(self.__notesFilePath, os.R_OK):
                ...
                if (bGotContent):
                    ...
                    onlyAnnotatorNotesLst = [record for record in recordSetLst if ("archive" not in record[
                        'message_type'])]  # i.e. do NOT count messages "archived" into notes file
        ...
        try:
            ...
            if (ok):
                ...
                for idx, record in enumerate(recordSetLst):
                    if (record['message_id'] == msgId):
                        msgAlreadySeen = True
                        mIIo.update('pdbx_deposition_message_status', 'action_reqd', p_msgStatusDict['action_reqd'],
                                    idx)
                        if (record[
                            'read_status'] == 'Y'):  # i.e. only pertinent updates of 'read_status' in this method are when user is setting read flag back to 'N' for unread
                            mIIo.update('pdbx_deposition_message_status', 'read_status', p_msgStatusDict['read_status'],
                                        idx)
                        mIIo.update('pdbx_deposition_message_status', 'for_release', p_msgStatusDict['for_release'],
                                    idx)
                        break
        ...
        try:
            ...
            if os.access(self.__msgsFrmDpstrFilePath, os.R_OK):
                ...
                if fileSizeBytes > 0:
                    with LockFile(self.__msgsFrmDpstrFilePath, timeoutSeconds=self.__timeoutSeconds,
                                  retrySeconds=self.__retrySeconds, verbose=self.__verbose,
                                  log=self.__lfh) as lf, FileSizeLogger(self.__msgsFrmDpstrFilePath,
                                                                        verbose=self.__verbose, log=self.__lfh) as fsl:
                        pid = os.getpid()
                        ok = mIIo.read(self.__msgsFrmDpstrFilePath, "msgingmod" + str(pid))
                    if (ok):
            ...
            if os.access(self.__msgsToDpstrFilePath, os.R_OK):
                ...
                if fileSizeToDpstr > 0:
                    with LockFile(self.__msgsToDpstrFilePath, timeoutSeconds=self.__timeoutSeconds,
                                  retrySeconds=self.__retrySeconds, verbose=self.__verbose,
                                  log=self.__lfh) as lf, FileSizeLogger(self.__msgsToDpstrFilePath,
                                                                        verbose=self.__verbose, log=self.__lfh) as fsl:
                        pid = os.getpid()
                        ok = mIIo2.read(self.__msgsToDpstrFilePath, "msgingmod" + str(pid))
                    if (ok):
            ...
            if self.__isWorkflow():
                if acronym == 'aux-file':
                    fPath = self.__reqObj.getValue(
                        "auxFilePath" + auxFileIndx)  # i.e. if aux-file then file is being provided by annotator and is NOT sourced from archive storage
                else:
                    fPath = msgDI.getFilePath(contentType, contentFormat)
                ...
                if (fPath is not None and os.access(fPath, os.R_OK)):
                    ...
                    bOk = self.__createAnnotateMilestone(p_msgObj.depositionId, p_msgObj.messageId, fPath, acronym,
                                                         contentType, contentFormat, auxFilePartNum, msgFileRefs,
                                                         failedMsgFileRefs)
                    ...
                    if (fileRef == 'model' and sIsEmMapOnly != "true"):
                        ...
                        bOk = self.__createModelReviewCopy(p_msgObj.depositionId, p_msgObj.messageId, fPath, acronym,
                                                           contentType, contentFormat, msgFileRefs, failedMsgFileRefs)
                    ...
                    if (fileRef == 'cs'):
                        ...
                        bOk = self.__createChemShiftsReviewCopy(p_msgObj.depositionId, p_msgObj.messageId, fPath,
                                                                acronym, contentType, "nmr-star", msgFileRefs,
                                                                failedMsgFileRefs)
                    ...
                    if (fileRef == 'nmr-data-str'):
                        ...
                        bOk = self.__createNmrDataStarReviewCopy(p_msgObj.depositionId, p_msgObj.messageId, fPath,
                                                                 acronym, contentType, "nmr-star", msgFileRefs,
                                                                 failedMsgFileRefs)
                ...
                for cntntTyp in generatedFilesList:
                    ...
                    msgFileRefs.append(
                        self.__createMsgFileReference(p_msgObj.messageId, p_msgObj.depositionId, cntntTyp,
                                                      contentFormat, annotPartitionNum, annotVersionNum, upldFileName))

        return bOk, msgFileRefs, failedMsgFileRefs

    def __createAnnotateMilestone(self, p_depId, p_msgId, fPath, acronym, contentType, contentFormat, auxFilePartNum,
                                  msgFileRefs, failedMsgFileRefs):
        ...
        bOk = True
        bEmdCnvrtRqrd = (
                self.__emDeposition and acronym == "model")  # i.e. need to convert model file into "emd" dialect for storage on deposition side
        ...
        dpstMilestoneFilePth = milestoneFilePthDict[
            'dpstPth']  # same filename as "archive" version of file, but different path for "deposit" area
        # last version
                ...
                if (dpstMilestoneFilePth is not None):
                    ...
                    if (bEmdCnvrtRqrd and self.__copyMilestoneDeposit):
                        ...
                        if (MessagingIo.bMakeEmXmlHeaderFiles == True):
                            bSuccess2 = self.__genAnnotMilestoneEmXmlHeader(p_depId, dpstMilestoneFilePth,
                                                                            dpstEmHeaderMilestoneFilePth) if dpstEmHeaderMilestoneFilePth else False
                        else:
                            bSuccess2 = True
                    ...
                    if ((self.__copyMilestoneDeposit and os.access(dpstMilestoneFilePth, os.R_OK)) or (
                            not self.__copyMilestoneDeposit)):
                        msgFileRefs.append(
                            self.__createMsgFileReference(p_msgId, p_depId, mileStoneCntntTyp, contentFormat,
                                                          annotPartitionNum, annotVersionNum, upldFileName))

                    else:
                        bOk = False
                        failedMsgFileRefs.append(mileStoneCntntTyp)
                        logger.error(
                            "problem with accessing dpstMilestoneFilePth if copy to deposit enabled: %s" % dpstMilestoneFilePth)
    ...
    def __createModelReviewCopy(self, p_depId, p_msgId, fPath, acronym, contentType, contentFormat, msgFileRefs,
                                failedMsgFileRefs):
        ##################################################################################
        ...
        if (reviewAnnotMilestoneFilePth is not None):
            ...
            if os.access(modelFileLocalPthAbslt, os.R_OK):
                ...
                #
                pdbxReviewFilePath = os.path.join(self.__sessionPath,
                                                  p_depId + "_model-review_P1.cif")  # filename here is arbitrary just for temporary session processing purposes
                ...
                if os.access(pdbxReviewFilePath, os.F_OK):
                    ...
                    if self.__skipCopyIfSame:
                        if self.__sameFile(pdbxReviewFilePath, reviewAnnotCurMilestoneFilePth):
                            logger.debug("Existing milestone good -- using %s %s" % (
                        ...
                        if self.__copyMilestoneDeposit:
                            if (os.access(reviewAnnotMilestoneFilePth, os.R_OK)):
                                ...
                                if (os.access(reviewDpstMilestoneFilePth, os.R_OK)):
                                    ...
                                    msgFileRefs.append(
                                        self.__createMsgFileReference(p_msgId, p_depId, reviewCntntTyp, contentFormat,
                                                                      annotPartitionNum, annotVersionNum))
                                else:
                                    bOk = False
                                    failedMsgFileRefs.append(reviewCntntTyp)
                                    logger.error(
                                        "problem with accessing deposit copy of 'review' milestone model file at: %s" % reviewDpstMilestoneFilePth)

                            else:
                                bOk = False
                                failedMsgFileRefs.append(reviewCntntTyp)
                                logger.error(
                                    "problem with accessing annotation copy of 'review' milestone model file at: %s" % reviewAnnotMilestoneFilePth)
                        else:
                    ...
                    if (self.__verbose):
                        logger.error(
                            "problem with accessing session copy of 'review' milestone model file at: %s" % pdbxReviewFilePath)

        logger.debug("FINISHED %s" % bOk)

        return bOk

    def __createChemShiftsReviewCopy(self, p_depId, p_msgId, fPath, acronym, contentType, contentFormat, msgFileRefs,
                                     failedMsgFileRefs):
        ##################################################################################
        ...
        if (reviewAnnotMilestoneFilePth is not None):
            ...
            if os.access(chemShiftsFileLocalPthAbslt, os.R_OK):
                ...
                # Generate Public pdbx cif file
                #
                nmrStarReviewFilePath = os.path.join(self.__sessionPath,
                                                     p_depId + "_cs-review_P1.cif")  # filename here is arbitrary just for temporary session processing purposes
    ...
    def __createNmrDataStarReviewCopy(self, p_depId, p_msgId, fPath, acronym, contentType, contentFormat, msgFileRefs,
                                      failedMsgFileRefs):
        ##################################################################################
        ...
        if os.access(reviewAnnotMilestoneFilePth, os.R_OK) and bOk == True:
            ...
            logger.info("About to generate NEF file %s -> %s", reviewAnnotMilestoneFilePth,
                        nefReviewAnnotMilestoneFilePth)

            logOutPath2 = os.path.join(self.__sessionPath,
                                       p_depId + "-logstrnef.json")  # output log for converted NEF file in "nmr-str2nef-release" op
            logOutPath1 = os.path.join(self.__sessionPath,
                                       p_depId + "-logstrstr.json")  # output log for converted NMR-STAR file in "nmr-str2nef-release" op
            ...
            if nexists:
                if self.__copyMilestoneDeposit:
                    ...
                    if (os.access(nefReviewDpstMilestoneFilePth, os.R_OK)):
                        ...
                        failedMsgFileRefs.append(reviewCntntTyp)
                        logger.error(
                            "problem with accessing deposit copy of 'review' milestone model file at: %s" % nefReviewDpstMilestoneFilePth)
                else:
                    # For not copying to deposit, register attachment
                    msgFileRefs.append(self.__createMsgFileReference(p_msgId, p_depId, reviewCntntTyp, contentFormat,
                                                                     annotPartitionNum, annotVersionNum))
            else:
                bOk = False
                failedMsgFileRefs.append(reviewCntntTyp)
                logger.error(
                    "problem with accessing annotation copy of 'review' milestone model file at: %s" % nefReviewAnnotMilestoneFilePth)
    ...
    def __genAnnotMilestoneEmdVrsn(self, p_depId, p_srcFilePath, p_dstFilePath):
        ...
        dpstModelEmdFilePth_Local = os.path.join(self.__sessionPath,
                                                 p_depId + "_model-emd_P1.cif")  # filename here is arbitrary just for temporary session processing purposes
    ...
    def __genAnnotMilestoneEmXmlHeader(self, p_depId, p_srcFilePath, p_dstFilePath):
        ...
        dpstModelEmHdrFilePth_Local = os.path.join(self.__sessionPath,
                                                   p_depId + "_em-volume-header_P1.xml")  # filename here is arbitrary just for temporary session processing purposes
    ...
    def __createMsgFileReference(self, p_msgId, p_depId, p_contentType, p_contentFormat, p_annotPartitionNum,
                                 p_annotVersionNum, p_upldFileName=None, p_storageType="archive"):
    ...
    def __sendNotificationEmail(self, p_msgObj, p_bVldtnRprtFlg=False):
        ...
        commHostName = self.__reqObj.getValue(
            "hostname")  # the hostname of site currently hosting the annotator's communication UI
        #
        ...
        for index, (emailAddrs, role, lname) in enumerate(contactAuthors):
            ...
            greetRecipientList += (
                    separator + (("Dr. " + lname) if (lname is not None and (len(lname) > 1)) else "Sir or Madam"))
        #
        msgStrDict['tab'] = "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;"
        #
        msgStrDict['dep_id'] = depId
        msgStrDict['pdb_block'] = (MessagingTemplates.emailNotif_pdbBlock % pdbId) if (
                pdbId is not None and pdbId != '[PDBID NOT AVAIL]') else ""
        msgStrDict['entry_authors'] = ', '.join(entryAuthors) if entryAuthors is not None else ""
        msgStrDict['entry_authors_block'] = MessagingTemplates.emailNotif_entryAuthorsBlock % msgStrDict if len(
            msgStrDict['entry_authors']) > 1 else ""
        ...
        msgStrDict['entry_title_block'] = MessagingTemplates.emailNotif_entryTitleBlock % msgStrDict if (
                len(msgStrDict['entry_title']) > 1 and msgStrDict[
            'entry_title'] != '[NOT AVAILABLE]') else "<br /><br />"
        ...
        if sys.version_info[0] < 3:
            ...
            msgStrDict['orig_commui_msg_content'] = msgStrDict['orig_commui_msg_content'].decode('utf-8').encode(
                'ascii', 'replace')
        #
        msgStrDict[
            'msg_body_main'] = MessagingTemplates.emailNotif_msgBodyMain % msgStrDict if not p_msgObj.isAutoMsg else (
        ...
        #
        msgStrDict[
            'subject'] = subject + " - DEP ID: " + p_msgObj.depositionId if not p_msgObj.isAutoMsg else p_msgObj.messageSubject
    ...
    def __trnsfrmFileRefDictToLst(self, p_recordSetLst, p_msgIdFilter=None):
        ...
        for rcrd in p_recordSetLst:
            ...
            try:
                ...
                msgDI = MessagingDataImport(self.__reqObj, verbose=self.__verbose, log=self.__lfh)
                pathDict = msgDI.getMileStoneFilePaths(contentType=contentType, format=contentFormat, version=versionId,
        ...
        for rowIdx, row in enumerate(p_recordSetLst):
            ...
            if (len(parentId) > 0 and parentId != msgId):
                ...
                try:
                    ...
                    if (prevMsgId != parentId) and (prevMsgParentId == parentId) and (
                            p_indentDict[msgId] == p_indentDict[prevMsgId]):
                        # if we're inside this if block, then we know current record is sibling of previous record

                        if datetime.strptime(prevMsgTimeStamp, '%Y-%m-%d %H:%M:%S') > datetime.strptime(timeStamp,
                                                                                                        '%Y-%m-%d %H:%M:%S'):
                            ...
                            if (self.__verbose and self.__debug and False):
                                self.__lfh.write(
                                    "\n\n+%s.%s() rowIdx is [%s] -- msgId is [%s] -- prevMsgId is [%s] -- indentlevel is [%s] -- indentPrev is [%s] \n" % (
                                        self.__class__.__name__, sys._getframe().f_code.co_name, rowIdx, msgId,
                                        prevMsgId,
                                        p_indentDict[msgId], p_indentDict[prevMsgId]))
                                self.__lfh.write("\n\n+%s.%s() new index is [%s] \n" % (
                            ...
                            if (p_indentDict[msgId] >= p_indentDict[prevMsgId]):
                                ...
                                if (self.__verbose and self.__debug):
                                    self.__lfh.write(
                                        "\n\n+%s.%s() rowIdx is [%s] -- msgId is [%s] -- prevMsgId is [%s] -- indentlevel is [%s] -- indentPrev is [%s] \n" % (
                                            self.__class__.__name__, sys._getframe().f_code.co_name, rowIdx, msgId,
                                            prevMsgId, p_indentDict[msgId], p_indentDict[prevMsgId]))
                                    self.__lfh.write("\n\n+%s.%s() new index is [%s] \n" % (
    ...
    def __isWorkflow(self):
        ...
        #
        if fileSource and fileSource in ['archive', 'autogroup', 'wf-archive', 'wf_archive', 'wf-instance',
                                         'wf_instance']:
...
class MsgTmpltHlpr(object):
    ...
    def populateTmpltDict(self, p_returnDict):
        ...
        p_returnDict['pdb_id'] = self.__pdbId if (
                self.__pdbId is not None and len(self.__pdbId) > 0 and self.__pdbId != '?') else "[PDBID NOT AVAIL]"
        p_returnDict['emdb_id'] = self.__emdbId if (self.__emdbId is not None and len(
            self.__emdbId) > 0 and self.__emdbId != '?') else "[EMDBID NOT AVAIL]"
        ...
        p_returnDict['entry_authors_newline_list'] = "\n".join(self.__entryAuthrs) if (
                self.__entryAuthrs is not None and len(self.__entryAuthrs) > 0) else "[NOT AVAILABLE]"
        p_returnDict['entry_authors_csv_list'] = ", ".join(self.__entryAuthrs) if (
                self.__entryAuthrs is not None and len(self.__entryAuthrs) > 0) else "[NOT AVAILABLE]"
        #
        p_returnDict['title'] = self.__title if (
                self.__title is not None and len(self.__title) > 0) else "[NOT AVAILABLE]"
        #
        p_returnDict['status_code'] = self.__statusCode if (
                self.__statusCode is not None and len(self.__statusCode) > 0) else "[NOT AVAILABLE]"
        p_returnDict['entry_status'] = self.__entryStatus if (
                self.__entryStatus is not None and len(self.__entryStatus) > 0) else "[NOT AVAILABLE]"
        p_returnDict['auth_rel_status_code'] = self.__authRelStatusCode if (
                self.__authRelStatusCode is not None and len(self.__authRelStatusCode) > 0) else "[NOT AVAILABLE]"
        p_returnDict['expire_date'] = self.__expireDate
        p_returnDict['recvd_date'] = du.date_to_display(self.__initRecvdDate) if (
                self.__initRecvdDate is not None and len(self.__initRecvdDate) > 0 and self.__isNotCifNull(
            self.__initRecvdDate)) else "[NOT AVAILABLE]"
        p_returnDict['processing_site'] = self.__procSite if (self.__procSite is not None and len(
            self.__procSite) > 0) else "[NOT AVAILABLE]"  # processing site used in statement regarding upcoming Thursday cutoff date
        #
        p_returnDict['citation_authors'] = ",".join(self.__citAuthors) if (
                self.__citAuthors is not None and len(self.__citAuthors) > 0) else "[NOT AVAILABLE]"
        p_returnDict['citation_title'] = self.__citTitle if (
                self.__citTitle is not None and len(self.__citTitle) > 0 and self.__isNotCifNull(
            self.__citTitle)) else "[NOT AVAILABLE]"
        p_returnDict['citation_journal_abbrev'] = self.__citJournalAbbrev if (
                self.__citJournalAbbrev is not None and len(self.__citJournalAbbrev) > 0 and self.__isNotCifNull(
            self.__citJournalAbbrev)) else "[NOT AVAILABLE]"
        p_returnDict['citation_journal_volume'] = ("\nVolume:   " + self.__citJournalVolume) if (
                self.__citJournalVolume is not None and len(self.__citJournalVolume) > 0 and self.__isNotCifNull(
            self.__citJournalVolume)) else ""
        p_returnDict['citation_page_first'] = self.__citPageFirst if (
                self.__citPageFirst is not None and len(self.__citPageFirst) > 0 and self.__isNotCifNull(
            self.__citPageFirst)) else "[NOT AVAILABLE]"
        p_returnDict['citation_page_last'] = (" - " + self.__citPageLast) if (
                self.__citPageLast is not None and len(self.__citPageLast) > 0 and self.__isNotCifNull(
            self.__citPageLast)) else ""
        p_returnDict['citation_pages'] = ("\nPages:   %(citation_page_first)s%(citation_page_last)s" % p_returnDict) if \
            p_returnDict['citation_page_first'] != "[NOT AVAILABLE]" else ""
        p_returnDict['citation_year'] = ("\nYear:   " + self.__citYear) if (
                self.__citYear is not None and len(self.__citYear) > 0 and self.__isNotCifNull(
            self.__citYear)) else ""
        p_returnDict['citation_pubmedid'] = ("\nPubMed ID:   " + self.__citPdbxDbIdPubMed) if (
                self.__citPdbxDbIdPubMed is not None and len(self.__citPdbxDbIdPubMed) > 0 and self.__isNotCifNull(
            self.__citPdbxDbIdPubMed)) else ""
        p_returnDict['citation_doi'] = ("\nDOI:   " + self.__citPdbxDbIdDOI) if (
                self.__citPdbxDbIdDOI is not None and len(self.__citPdbxDbIdDOI) > 0 and self.__isNotCifNull(
            self.__citPdbxDbIdDOI)) else ""
        #
        p_returnDict['default_msg_tmplt'] = self.__defaultMsgTmpltType
        #
        p_returnDict['outbound_rprt_date'] = self.__lastOutboundRprtDate if (
                self.__lastOutboundRprtDate is not None and len(
            self.__lastOutboundRprtDate) > 0) else "[NOT AVAILABLE]"
        #
        p_returnDict['release_date'] = self.__releaseDate if (
                self.__releaseDate is not None and len(self.__releaseDate) > 0) else "[NOT AVAILABLE]"
        p_returnDict['withdrawn_date'] = p_returnDict['release_date']
        p_returnDict['thurs_prerelease_clause'] = self.__thursPreRlsClause if (
                self.__thursPreRlsClause is not None and len(self.__thursPreRlsClause) > 0) else ""
        p_returnDict['thurs_wdrn_clause'] = self.__thursWdrnClause if (
                self.__thursWdrnClause is not None and len(self.__thursWdrnClause) > 0) else ""
        ...
        p_returnDict['unlock_date'] = du.date_to_display(self.__lastUnlockDate) if (
                self.__lastUnlockDate is not None and len(self.__lastUnlockDate) > 0) else "[NOT AVAILBLE]"
        ...
        if self.__emDeposition and self.__emMapOnly:
            ...
            p_returnDict[
                'doinotice'] = 'Your entry has been assigned the following PDB DOI: https://doi.org/10.2210/pdb{}/pdb\n\nPlease use the above PDB DOI link to access your data.\n'.format(
                p_returnDict['pdb_id'])
        ...
        if (self.__emDeposition):
            rqstdNumber = "multi" if (len(
                accessionIdList) > 1) else "single"  # i.e. number of accession IDs requested, varies depending on possible presence of model accompanying the map
            #
            ...
            if (self.__emMapOnly):
                p_returnDict['em_entry_authors_newline_list'] = "\n".join(self.__emEntryAuthrs) if (
                        len(self.__emEntryAuthrs) > 0) else "[NOT AVAILABLE]"
                p_returnDict['em_title'] = self.__emTitle if (
                        self.__emTitle is not None and len(self.__emTitle) > 0) else "[NOT AVAILABLE]"
            elif (self.__emMapAndModelEntry):
                if (self.__emSameAuthorsAsPDB == "no"):
                    p_returnDict['em_entry_authors_newline_list'] = "EMDB:\n" + "\n".join(self.__emEntryAuthrs) if (
                            len(self.__emEntryAuthrs) > 0) else "[NOT AVAILABLE]"
                    p_returnDict['em_entry_authors_newline_list'] = p_returnDict[
                                                                        'em_entry_authors_newline_list'] + "\n\nPDB:\n" + "\n".join(
                        self.__entryAuthrs) if (len(self.__entryAuthrs) > 0) else "[NOT AVAILABLE]"
                else:
                    # i.e map-and-model entry with EM authors same as PDB authors
                    p_returnDict['em_entry_authors_list'] = self.__entryAuthrs
                    p_returnDict['em_entry_authors_newline_list'] = "\n".join(self.__entryAuthrs) if (
                            len(self.__entryAuthrs) > 0) else "[NOT AVAILABLE]"
                    p_returnDict['em_entry_authors_csv_list'] = ", ".join(self.__entryAuthrs) if (
                            len(self.__entryAuthrs) > 0) else "[NOT AVAILABLE]"

                if (self.__emSameTitleAsPDB == "no"):
                    p_returnDict['em_title'] = ("EMDB: " + self.__emTitle) if (
                            self.__emTitle is not None and len(self.__emTitle) > 0) else "EMDB: [NOT AVAILABLE]"
                else:
                    # i.e map-and-model entry with EM title same as PDB title
                    p_returnDict['em_title'] = self.__title if (
                            self.__title is not None and len(self.__title) > 0) else "[NOT AVAILABLE]"
            ...
            #
            p_returnDict['status_code_em_map'] = self.__statusCodeEmMap if (
                    self.__statusCodeEmMap is not None and len(self.__statusCodeEmMap) > 0) else "[NOT AVAILABLE]"
            p_returnDict['entry_status_em_map'] = self.__entryStatusEmMap if (
                    self.__entryStatusEmMap is not None and len(self.__entryStatusEmMap) > 0) else "[NOT AVAILABLE]"
            ...
            #
            p_returnDict['outbound_rprt_date_em'] = self.__lastOutboundRprtDateEm if (
                    self.__lastOutboundRprtDateEm is not None and len(
                self.__lastOutboundRprtDateEm) > 0) else "[NOT AVAILABLE]"
            #
            p_returnDict[
                'caveat_records'] = " with CAVEAT records highlighting any outstanding issues" if self.__emMapAndModelEntry else ""
            ...
            # message template closing details
            p_returnDict[
                'annotator_group_signoff'] = MessagingTemplates.msgTmplt_annotatorGroupSignoff if self.__emModelOnly else MessagingTemplates.msgTmplt_annotatorGroupSignoff_em % p_returnDict
        ...
        ########### TITLE PROCESSING ################################
        #############################################################
        titleLength = len(p_returnDict['title']) if (not self.__emDeposition) else (
            len(p_returnDict['em_title']) if (len(p_returnDict['em_title']) > len(p_returnDict['title'])) else len(
                p_returnDict['title']))
        ...
        if (titleLength <= 100):
            ...
            for num in range(targetLength, targetLength - 20, -1):
                ...
                if (len(textwrap.wrap(primaryTitle, num)[
                            -1]) > 10):  # EM: using reference to primaryTitle instead of p_returnDict['title']
                    targetLength = num
                    if (self.__debugLvl2):
                        self.__lfh.write(
                            "+%s.%s() found targetLength of:  %s\n" % (className, methodName, targetLength))
                    break
            if (self.__debugLvl2):
        ...
        if (self.__emDeposition):
            if (self.__emMapAndModelEntry and self.__emSameTitleAsPDB == "no"):
                ...
                p_returnDict['em_title'] = p_returnDict['em_title'] + ("\nPDB: " + p_returnDict['title']) if (
                        p_returnDict['title'] is not None and len(
                    p_returnDict['title']) > 0) else "&#13;&#10;PDB: [NOT AVAILABLE]"
    ...
    def __getContactAuthors(self):
        ...
        brainPageContactList = ss.runSelectNQ(table='user_data', select=['email', 'role', 'last_name'],
                                              where={"dep_set_id": self.__depId, "role": roleFilter})

        if (self.__depSystemVrsn2 and len(
                brainPageContactList) == 0):  # if we have no brain contacts using Dep UI system 2.0 query, double check using pre vrsn 2.0 query qualifier for backwards compatibility
            brainPageContactList = ss.runSelectNQ(table='user_data', select=['email', 'role', 'last_name'],
    ...
    def __getEntryAuthors(self, p_IdType="PDB"):
        ...
        try:
            ...
            if (self.__verbose):
                self.__lfh.write(
                    "+%s.%s() problem recovering data from PdbxPersist for category: '%s'\n" % (self.__class__.__name__,
                                                                                                sys._getframe().f_code.co_name,
                                                                                                ctgryNm))
            traceback.print_exc(file=self.__lfh)
            self.__lfh.flush()
    ...
    def __getProcessingStatusInfo(self):
        ...
        try:
            ...
            if catObj:
                ...
                du = DateUtil()
                self.__holdDate = "[none listed]" if (
                        self.__holdDate is None or len(self.__holdDate) < 1 or self.__isCifNull(
                    self.__holdDate)) else \
                ...
                if (self.__authRelStatusCode.upper() == 'HOLD'):
                    ...
                    if (self.__initRecvdDate is not None and len(self.__initRecvdDate) > 0 and self.__isNotCifNull(
                            self.__initRecvdDate)):
                        ...
                        if (initRecvdDate.month == 2 and initRecvdDate.day == 29):
                            initRecvdDatePlusOneYr = initRecvdDate.replace(year=(initRecvdDate.year + 1), month=3,
                                                                           day=1)
                        else:
    ...
    def __getProcessingStatusInfoEM(self):
        ...
        try:
            ...
            if catObj:
                ...
                if (self.__dpstnDateEmMap is not None and len(self.__dpstnDateEmMap) > 0 and self.__isNotCifNull(
                        self.__dpstnDateEmMap)):
    ...
    def __getCitationInfo(self):
        ...
        try:
            ...
            if catObj:
                ...
                idxPdbxDbIdDOI = itDict[
                    '_citation.pdbx_database_id_doi'] if '_citation.pdbx_database_id_doi' in itDict else None
                idxPdbxDbIdPubMed = itDict[
                    '_citation.pdbx_database_id_pubmed'] if '_citation.pdbx_database_id_pubmed' in itDict else None
        ...
        try:
            ...
            if ctgryObj:
                ...
                assert (iTotalRecords == 1), (
                        "+%s.%s() -- expecting '%s' category to contain a single record but had %s records\n" % (
    ...
    def __getReleaseDateInfo(self):
        ...
        if (notherDateRef.weekday() > 3 and notherDateRef.weekday() < 5) or (
                notherDateRef.weekday() == 3 and now.hour >= 11):  # i.e. if we're past the deadline for acceptable change requests
            self.__thursPreRlsClause = ""
            self.__thursWdrnClause = ""
        else:
            # i.e. if we're still before the deadline for acceptable change requests
            processingSite = self.__procSite if (
                    self.__procSite is not None and len(self.__procSite) > 0) else "[not available]"
            ...
            if (notherDateRef.weekday() != 3):
                ...
                self.__thursPreRlsClause = 'If you have changes to make to the entry, please inform us by noon local time at %s on Thursday %s.' % (
                    processingSite, du.datetime_to_display(notherDateRef))
                self.__thursWdrnClause = """
