...
from wwpdb.apps.msgmodule.db import (
    ...
    get_messaging_database_config, is_messaging_database_enabled
)
...
class MessagingIoDatabase:
    ...
    def __processMsg_database(self, p_msgObj):
        ...
        try:
            ...
            status_record = MessageStatus(**message_data['status']) if message_data['status'] else None
            file_references = [MessageFileReference(**fr) for fr in message_data['file_references']]
            ...
            if success and self.__verbose:
                logger.info(f"Successfully stored message {message_record.message_id} in database")
    ...
    def __extract_message_data(self, p_msgObj) -> Dict:
        ...
        message_data = {
            ...
            'status': {
                ...
                'deposition_data_set_id': p_msgObj.depositionId,
                'read_status': 'N',
                'action_reqd': 'Y' if getattr(p_msgObj, 'actionRequired', False) else 'N',
                'for_release': 'N'
            } if p_msgObj.isBeingSent else None,
            'file_references': []
        }
    ...
    def getMsgRowList(self, p_depDataSetId, p_sSendStatus="Y", p_bServerSide=False,
                     p_iDisplayStart=None, p_iDisplayLength=None, p_sSrchFltr=None,
                     p_colSearchDict=None, p_bThreadedRslts=False):
        """
        ...
        try:
            if self.__db_service:
                return self.__getMsgRowList_database(
                    p_depDataSetId, p_sSendStatus, p_bServerSide,
                    p_iDisplayStart, p_iDisplayLength, p_sSrchFltr, p_colSearchDict
                )
            else:
    ...
    def __getMsgRowList_database(self, depId, sendStatus, serverSide=False,
                                displayStart=None, displayLength=None,
                                searchFilter=None, colSearchDict=None):
        ...
        try:
            ...
            for msg_data in messages:
                ...
                record = [
                    ...
                    msg.message_subject,
                    msg.message_text[:100] + '...' if len(msg.message_text) > 100 else msg.message_text,  # Truncated for list view
                    msg.message_type,
                    msg.send_status
                ]
    ...
    def __markMsgAsRead_database(self, msgStatusDict):
        ...
        try:
            ...
            success = self.__db_service.status_dao.mark_message_read(message_id, read_status)
