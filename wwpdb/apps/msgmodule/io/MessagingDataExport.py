...
class MessagingDataExport(object):
    ...
    def getFilePath(self,contentType,format):
        return self.__getWfFilePath(contentType=contentType,format=format,fileSource=self.__fileSource,version='latest')

    def getFilePathExt(self,contentType,format,fileSource = 'deposit', version = 'latest'):
        return self.__getWfFilePath(contentType = contentType,format = format, fileSource= fileSource ,version = version)
    
    #########################--Milestone File Handling--########################################################
    def getMileStoneFilePaths(self,contentType,format,partitionNum=None):
        pathDict={}
        
        try:
            curFilePth = self.__getWfFilePath(contentType=contentType,format=format,fileSource="archive",version='latest',partitionNum=partitionNum)
            ...
            annotFilePth = self.__getWfFilePath(contentType=contentType,format=format,fileSource="archive",version='next',partitionNum=partitionNum)
            dpstFileNm = os.path.basename(annotFilePth)
            dpstFilePth = os.path.join(self.__dpstStoragePath,dpstFileNm)
            
        except:
            logger.exception("ERROR on getting milestone paths for contentType: '%s', format/file extension: '%s'\n" % (contentType,format))
    ...
    def __getWfFilePath(self,contentType,format='pdbx',fileSource='deposit',version='next',partitionNum=None):
        try:
    ...
    def __getWfFilePathRef(self,contentType,format='pdbx',fileSource='deposit',version='next',partitionNum=None):
