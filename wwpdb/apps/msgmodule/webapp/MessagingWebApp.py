...
class MessagingWebAppWorker(object):
    ...
    def _getDataTblConfigDtls(self):
        ...
        rsltSetDict = msgingIo.getMsgRowList(p_depDataSetId=depId, p_sSendStatus=sendStatus, p_bServerSide=False,
                                             p_bThreadedRslts=bUseThreadedRsltSet)  # serverside False b/c only getting indentation info necessary for initial config
        #
        ...
        if ('CURRENT_NUM_MSGS_TO_DPSTR' in rsltSetDict):
            dtblConfigDict['CURRENT_NUM_MSGS_TO_DPSTR'] = rsltSetDict[
                'CURRENT_NUM_MSGS_TO_DPSTR']  # adding member representing total #records of msgs to depositor (live AND draft)
        if ('CURRENT_NUM_NOTES' in rsltSetDict):
            dtblConfigDict['CURRENT_NUM_NOTES'] = rsltSetDict[
                'CURRENT_NUM_NOTES']  # adding member representing total #records of notes (live AND draft)
        #
        if (bUseThreadedRsltSet and 'INDENT_DICT' in rsltSetDict):
            dtblConfigDict['INDENT_DICT'] = rsltSetDict[
                'INDENT_DICT']  # adding member representing threading/indent-formatting defined per msgid
        #
    ...
    def _getDataTblData(self):
        ...
        ########################################################################################################################################
        rsltSetDict = msgingIo.getMsgRowList(p_depDataSetId=depId, p_sSendStatus=sendStatus, p_bServerSide=True,
                                             p_iDisplayStart=iDisplayStart, p_iDisplayLength=iDisplayLength,
                                             p_sSrchFltr=sSearch, p_colSearchDict=colSearchDict,
    ...
    def _updateWfNotifyStatus(self, p_depId, p_status):
        ...
        try:
            ...
            if groupId:
                ...
                statusApi.runUpdate(table='batch_user_data', where={'dep_set_id': groupId.upper()},
                ...
                statusApi.runUpdate(table='deposition', where={'dep_set_id': p_depId.upper()},
                                    data={'notify': p_status})
            #
            bSuccess = True
        except:
    ...
    def _propagateMsg(self, actionType):
        ...
        self.__reqObj.setValue("content_type",
                               contentType)  # setting here for downstream processing (as opposed to "msgs")
        ...
        if ((bOk is True) and (contentType == "notes") and (self._getNotesFlaggingStatus() == 'false') and (
                flagMsg == "y")):
            self.__reqObj.setValue("activate_notes_flagging", 'true')
            self._toggleNotesFlagging()
            bGlobalStatusCheckReqd = True
        #
        if ((bOk is True) and (contentType == "notes") and (self._getNotesFlaggingStatus("bmrb") == 'false') and (
                flagMsg == "y")):
    ...
    def _processMsg(self, p_msgState):
        ...
        #
        self.__reqObj.setReturnFormat(return_format="json")
        self.__reqObj.setValue("message_state",
                               p_msgState)  # setting here for downstream processing, used only for processing purposes
    ...
    def __uploadFile(self, fileTag='file'):
        ...
        try:
            ...
            for cnt in range(1, 4):
                ...
                if (fileTag == auxFl):
                    fileType = os.path.splitext(fileName)[1].strip(".") if len(
                        os.path.splitext(fileName)[1]) > 1 else "n/a"
    ...
    def __updateWfTrackingDb(self, p_status):
        ...
        try:
            ...
            if (self.__verbose):
                self.__lfh.write(
                    "+MessagingWebAppWorker.__updateWfTrackingDb() -TRACKING status updated to '%s' for session %s \n" % (
                    p_status, sessionId))
        except:
            bSuccess = False
            if (self.__verbose):
                self.__lfh.write(
                    "+MessagingWebAppWorker.__updateWfTrackingDb() - TRACKING status, update to '%s' failed for session %s \n" % (
                    p_status, sessionId))
            traceback.print_exc(file=self.__lfh)
            #
        return bSuccess
