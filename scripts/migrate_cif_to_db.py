...
from wwpdb.apps.msgmodule.db import (
    MessagingDatabaseService, MessageRecord, MessageStatus, MessageFileReference
)
...
class CifToDbMigrator:
    ...
    def migrate_deposition_messages(self, deposition_id: str, test_mode: bool = False) -> bool:
    ...
    def migrate_bulk_depositions(self, deposition_list: List[str], test_mode: bool = False) -> Dict:
        ...
        logger.info(f"Bulk migration completed. Success: {len(successful)}, Failed: {len(failed)}")
        return results
    
    def migrate_from_directory(self, directory_path: str, test_mode: bool = False) -> Dict:
        ...
        logger.info(f"Directory migration completed. Success: {len(successful)}, Failed: {len(failed)}")
        return results
    
    def _migrate_cif_file(self, file_path: str, message_type: str, test_mode: bool = False) -> bool:
        ...
        try:
            ...
            # Parse CIF file
            messages, file_references, status_records = self._parse_cif_file(file_path, message_type)
            ...
            if not test_mode:
                success = self._store_parsed_data(messages, file_references, status_records)
                if not success:
                    self.stats['errors'] += 1
                    return False
            else:
                logger.info(f"TEST MODE: Would migrate {len(messages)} messages from {file_path}")
    ...
    def _parse_cif_file(self, file_path: str, message_type: str) -> tuple:
        ...
        try:
            ...
            logger.info(f"Parsed {len(messages)} messages, {len(file_references)} file refs, "
    ...
    def _convert_to_message_record(self, msg_info: Dict, message_type: str) -> MessageRecord:
    ...
    def _store_parsed_data(self, messages: List[MessageRecord], 
                          file_references: List[MessageFileReference],
                          status_records: List[MessageStatus]) -> bool:
        """Store parsed data in database"""
        try:
            # Store messages first
            for message in messages:
                # Check if message already exists
                existing = self.db_service.message_dao.get_message_by_id(message.message_id)
                if existing:
            ...
            for file_ref in file_references:
                if not self.db_service.file_dao.create_file_reference(file_ref):
                    logger.error(f"Failed to create file reference for message {file_ref.message_id}")
                    return False
            
            # Store status records
            for status in status_records:
                if not self.db_service.status_dao.create_or_update_status(status):
                    logger.error(f"Failed to create status for message {status.message_id}")
                    return False
    ...
    def _get_message_file_path(self, deposition_id: str, message_type: str) -> Optional[str]:
        ...
        try:
            ...
            logger.error(f"Error getting file path for {deposition_id}, {message_type}: {e}")
            return None
    ...
    def _is_message_cif_file(self, filename: str) -> bool:
        ...
        return any(pattern in filename for pattern in message_patterns) and filename.endswith('.cif')
...
def main():
    ...
    parser.add_argument('--depositions-file', help='File containing list of deposition IDs')
    parser.add_argument('--directory', help='Directory containing CIF files to migrate')
    parser.add_argument('--test-mode', action='store_true', help='Run in test mode (no database writes)')
    parser.add_argument('--site-id', default='RCSB', help='Site ID for file path resolution')
    ...
    try:
        if args.verify:
            ...
            success = migrator.migrate_deposition_messages(args.deposition, args.test_mode)
            print(f"Migration {'successful' if success else 'failed'} for {args.deposition}")
            ...
            print(f"Bulk migration completed: {len(results['successful'])} successful, {len(results['failed'])} failed")
            
        elif args.directory:
            # Directory migration
            results = migrator.migrate_from_directory(args.directory, args.test_mode)
            print(f"Directory migration completed: {len(results['successful'])} successful, {len(results['failed'])} failed")
